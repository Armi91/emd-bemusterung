...
export class AuthService {
  ...
  getCurrentUser() {
    return authState(this.auth).pipe(
      switchMap(user => {
        if (user) {
          return <Observable<UserData>>docData(doc(this.firestore, `users/${user.uid}`));
        } else {
          return of(null);
        }
      })
  ...
  async register(userForm: UserData) {
    return createUserWithEmailAndPassword(this.auth, userForm.email, userForm.password!).then(userCredential => {
