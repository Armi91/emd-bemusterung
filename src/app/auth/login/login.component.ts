...
export class LoginComponent implements OnInit {
    ...
    this.store
      ...
      .subscribe((err) => {
        if (err) {
          ...
          this.toastr.error(this.errorTranslateSrv.getMessage(err.code), 'Login-Fehler');
        }
      });
    ...
    this.store
      ...
      .subscribe((whereToGo) => {
        if (whereToGo === 'toRooms') {
          ...
          this.router.navigate(['/c/room/86P0cGHDtm1fe3nK14Th/edit']);
          // this.router.navigate(['/c/rooms']);
          // this.router.navigate(['/c/preview/3XN0gt00Ktf2pm18jO6n']);
  ...
  login(e: SubmitEvent, form: NgForm) {
    if (form.form.valid) {
      ...
      this.toastr.error('Geben Sie die korrekten Login-Daten ein.', 'Login-Fehler');
    }
  }
}
